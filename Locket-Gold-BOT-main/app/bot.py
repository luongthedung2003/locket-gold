...
async def stats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    stats = db.get_stats()
    msg = (
        f"{E_STAT} <b>THỐNG KÊ HỆ THỐNG</b>\n"
        f"━━━━━━━━━━━━━━━━━━━\n"
        f"{E_USER} <b>Người dùng</b>: {stats['unique_users']}\n"
        f"{E_GLOBE} <b>Tổng yêu cầu</b>: {stats['total']}\n"
        f"{E_SUCCESS} <b>Thành công</b>: {stats['success']}\n"
        f"{E_ERROR} <b>Thất bại</b>: {stats['fail']}\n"
        f"━━━━━━━━━━━━━━━━━━━\n"
        f"{E_ANDROID} <b>Workers đang chạy</b>: {NUM_WORKERS}\n"
        f"🔑 <b>Bộ Token</b>: {len(TOKEN_SETS)}\n"
        f"⏳ <b>Hàng chờ</b>: {request_queue.qsize()}\n"
    )
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Nếu đã có Gold → thông báo và gửi DNS
    if status and status.get("active"):
        status_text = T("gold_active", lang).format(status['expires'])
        
        # Tạo DNS profile
        pid, link = await nextdns.create_profile(NEXTDNS_KEY)
        ...
        await msg.edit_text(
            f"{E_SUCCESS} <b>Bạn đã có Gold rồi!</b>\n\n"
            f"{E_ID}: <code>{uid}</code>\n"
            f"{E_TAG}: <code>{username}</code>\n"
            f"{E_STAT} <b>Trạng thái</b>: {status_text}\n\n"
            f"{E_SHIELD} <b>Chỉ cần cài DNS để giữ Gold vĩnh viễn:</b>\n"
            f"{dns_text}",
            parse_mode=ParseMode.HTML,
            disable_web_page_preview=True
        )
        
        # Gửi file .mobileconfig
        if pid:
            try:
                import os
                mobileconfig_path = nextdns.generate_mobileconfig(pid, username)
                with open(mobileconfig_path, 'rb') as f:
                    await update.message.reply_document(
                        document=f,
                        filename=f"DNS_{username}.mobileconfig",
                        caption=f"{E_SHIELD} <b>File cài DNS cho iOS</b>\n📲 Tải file → Mở bằng Safari → Cài đặt Profile",
                        parse_mode=ParseMode.HTML
                    )
                os.remove(mobileconfig_path)
            except Exception as e:
                logger.error(f"Send mobileconfig error: {e}")
        return
    
    # Chưa có Gold → hiện nút kích hoạt
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_upgrade":
        if db.is_premium(user_id):
            await query.answer("Bạn đã là Premium rồi!", show_alert=True)
            return
        ...
        await query.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("💳 Thanh toán ngay", callback_data="buy_premium")],
    ...
    if data == "buy_premium":
        # Generate unique memo: LOCKET + user_id + random
        memo = f"LOCKET{user_id}{random.randint(10, 99)}"
        amount = PREMIUM_PRICE
        
        db.create_order(user_id, amount, memo)
        
        qr_url = sepay.generate_qr_url(amount, memo)
        instruction = T("payment_instruction", lang).format(amount, memo)
        
        # Try to send photo by URL first
        try:
            await context.bot.send_photo(
                ...
                caption=instruction,
                parse_mode=ParseMode.HTML,
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Menu chính", callback_data="menu_back")]])
            )
            await query.message.delete()
        except Exception as e:
            logger.error(f"Send QR URL error: {e}")
            # Fallback: Try downloading image and sending as file
            try:
                response = req.get(qr_url, timeout=10)
                if response.status_code == 200:
                    from io import BytesIO
                    photo_file = BytesIO(response.content)
                    photo_file.name = "qr.png"
                    await context.bot.send_photo(
                        chat_id=user_id,
                        photo=photo_file,
                        caption=instruction,
                    ...
                    raise Exception(f"QR Download failed: {response.status_code}")
            except Exception as e2:
                logger.error(f"Send QR File error: {e2}")
                await query.message.edit_text(
                    instruction + "\n\n(Không thể tải mã QR, vui lòng chuyển khoản theo thông tin trên)",
                    parse_mode=ParseMode.HTML,
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Menu chính", callback_data="menu_back")]])
                )
        return
    ...
    while True:
        try:
            ...
            if success:
                ...
                # Gửi file .mobileconfig tùy chỉnh qua Telegram
                if pid:
                    try:
                        import os
                        mobileconfig_path = nextdns.generate_mobileconfig(pid, username)
...
def get_main_menu_keyboard(lang):
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton(T("btn_upgrade_menu", lang), callback_data="menu_upgrade")],
