...
@section('title', 'Chào mừng quay trở lại - Locket Gold')
    ...
    <div class="yetti-card">
        ...
        <div class="yetti-hero">
            ...
            <div class="hero-overlay-text">
                ...
                <p class="hero-sub">Mở khóa toàn bộ đặc quyền Locket Gold ngay hôm nay. Trải nghiệm không giới hạn, bảo hành trọn đời và hỗ trợ 24/7 từ đội ngũ chuyên gia.</p>
                <div class="hero-features">
                    <div class="h-feat"><iconify-icon icon="solar:check-circle-bold"></iconify-icon> Video 3s - 60s</div>
                    <div class="h-feat"><iconify-icon icon="solar:check-circle-bold"></iconify-icon> Đổi Icon ứng dụng</div>
                    <div class="h-feat"><iconify-icon icon="solar:check-circle-bold"></iconify-icon> Badge vàng chính chủ</div>
                </div>
            </div>
        </div>
        ...
        <div class="yetti-form-side">
            ...
            <div class="yetti-form-header">
                ...
                <p>Nhập email và mật khẩu của bạn để truy cập tài khoản</p>
            </div>

            <form action="{{ route('login') }}" method="POST" class="yetti-form">
                @csrf
                ...
                <div class="y-options">
                    ...
                    <a href="#" class="y-forgot">Quên mật khẩu?</a>
                </div>

                <button type="submit" class="y-btn-black">Đăng nhập</button>
    ...
    <div id="comingSoonModal" class="y-modal-overlay" style="display: none;" onclick="closeYettiModal(event)">
        <div class="y-modal-content" onclick="event.stopPropagation()">
            ...
            <p class="y-modal-desc">
                Chức năng đăng nhập bằng Google đang được chúng tôi xây dựng và hoàn thiện. Vui lòng quay lại sau hoặc sử dụng tài khoản email để đăng nhập.
            </p>
...
.yetti-page-container {
    ...
    align-items: center;
    height: calc(100vh - 100px); /* Vừa khít màn hình, trừ đi thanh nav */
    padding: 20px;
    background: #f8fafc;
}
...
.yetti-card {
    ...
    width: 100%;
    max-width: 1100px;
    height: auto; /* Để tự co giãn theo nội dung */
    min-height: 650px;
    max-height: 850px; /* Tăng chiều dài để không bị che */
    background: #ffffff;
    border-radius: 32px;
    overflow: hidden;
...
.hero-overlay-text {
    position: absolute;
    bottom: 0; /* Cho lớp phủ bắt đầu từ sát đáy */
    left: 0;
    right: 0;
    padding: 40px 40px 90px 40px; /* Tăng padding bottom để đẩy chữ lên thay vì dùng bottom */
...
.hero-overlay-text h1 {
    ...
    font-weight: 900;
    font-size: 2.5rem; /* Bé lại một chút */
    color: white;
    line-height: 1.1;
    margin: 0 0 10px 0;
}
...
.h-feat iconify-icon { color: #4ade80; font-size: 1.1rem; }
...
.left-eye { top: 100px; left: 60px; } /* Đưa lên khoảng trống phía trên */
...
.yetti-form-header p {
    color: #64748b;
    margin-bottom: 30px;
    font-size: 0.85rem; /* Bé lại */
}

.y-input-group {
    margin-bottom: 18px; /* Giảm gap */
}
...
.y-input-group input {
    width: 100%;
    padding: 14px 18px; /* Giảm padding */
    background: #f1f5f9;
    border: none;
    border-radius: 12px;
    font-size: 0.9rem; /* Bé lại */
    outline: none;
    transition: all 0.2s;
}
html:not(.light) .y-input-group input { background: #1a1a1a; color: white; }

.y-input-group input:focus {
...
html:not(.light) .y-forgot { color: #fff; }
...
@media (max-width: 992px) {
    .yetti-page-container { height: auto; min-height: 100vh; padding: 0; } /* Bỏ padding */
    .yetti-card { flex-direction: column; border-radius: 0; height: auto; max-height: none; box-shadow: none; } /* Bỏ border radius để sát lề */
    .yetti-hero { height: 350px; flex: none; }
    .hero-overlay-text { bottom: 0; padding: 20px 20px 40px 20px; } /* Đưa xuống sát đáy */
    .hero-overlay-text h1 { font-size: 2rem; }
    .hero-sub { display: none; }
    .yetti-form-side { padding: 30px 20px; }
    
    /* Reposition eyes for mobile */
