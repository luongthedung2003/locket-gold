...
def is_premium(user_id):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute("SELECT is_premium FROM user_settings WHERE user_id = ?", (user_id,))
    result = c.fetchone()
    conn.close()
...
def set_premium(user_id, status=1):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute("INSERT INTO user_settings (user_id, is_premium) VALUES (?, ?) ON CONFLICT(user_id) DO UPDATE SET is_premium = ?", (user_id, status, status))
    conn.commit()
    conn.close()

def create_order(user_id, amount, memo):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute("INSERT INTO orders (user_id, amount, memo) VALUES (?, ?, ?)", (user_id, amount, memo))
    conn.commit()
    conn.close()
...
def get_order_by_memo(memo):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute("SELECT user_id, amount, status FROM orders WHERE memo = ?", (memo,))
    result = c.fetchone()
    conn.close()
    return result

def update_order_status(memo, status):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute("UPDATE orders SET status = ? WHERE memo = ?", (status, memo))
    conn.commit()
    conn.close()
