...
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8724805474:AAHJjn-3nxKugrp3D3OfEIu50hzdjG6DcAI")
NEXTDNS_KEY = os.environ.get("NEXTDNS_KEY", "f96c8499069c4d219f190924efabf87eb4bf2682")
TOKEN_SETS = [
    {
...
# --- SePay Configuration ---
SEPAY_API_KEY = os.environ.get("SEPAY_API_KEY", "YOUR_SEPAY_API_KEY")
SEPAY_WEBHOOK_KEY = os.environ.get("SEPAY_WEBHOOK_KEY", "sepay_secret_key_123456") # Copy this to SePay Webhook Security tab
BANK_ACCOUNT = os.environ.get("BANK_ACCOUNT", "3801062003")
BANK_NAME = os.environ.get("BANK_NAME", "MB")
PREMIUM_PRICE = 2000 # VND
CURRENCY = "VND"
...
TEXTS = {
    "VI": {
        ...
        "btn_help": "🆘 Hỗ Trợ",
        "btn_upgrade_menu": "🚀 Nâng Cấp Gold (Premium)",
        "premium_msg": (
            f"<b>{E_MENU} NÂNG CẤP PREMIUM</b>\n\n"
            f"💎 <b>Quyền lợi:</b>\n"
            f"- Không giới hạn lượt kích hoạt/ngày\n"
            f"- Ưu tiên xử lý nhanh hơn\n"
            f"- Hỗ trợ vĩnh viễn\n\n"
            f"💰 <b>Giá:</b> <code>{2000} VND</code> / Vĩnh viễn\n\n"
            f"👇 Bấm nút bên dưới để nhận mã thanh toán."
        ),
        "payment_instruction": (
            f"<b>{E_LOADING} THÔNG TIN THANH TOÁN</b>\n\n"
            f"🏦 <b>Ngân hàng:</b> {BANK_NAME}\n"
            f"👤 <b>Số tài khoản:</b> <code>{BANK_ACCOUNT}</code>\n"
            f"💰 <b>Số tiền:</b> <code>{{}} VND</code>\n"
            f"📝 <b>Nội dung (MEMO):</b> <code>{{}}</code>\n\n"
            f"⚠️ <b>LƯU Ý:</b>\n"
            f"1. Phải chuyển đúng <b>Số tiền</b> và <b>Nội dung</b> trên.\n"
            f"2. Hệ thống sẽ tự động nâng cấp sau 1-3 phút.\n"
            f"3. Quét mã QR bên dưới để thanh toán nhanh hơn."
        ),
        "payment_success": f"{E_SUCCESS} <b>THANH TOÁN THÀNH CÔNG!</b>\nTài khoản của bạn đã được nâng cấp lên <b>Premium</b>. Bây giờ bạn có thể sử dụng không giới hạn lượt!",
    ...
    "EN": {
        ...
        "btn_help": "🆘 Help",
        "btn_upgrade_menu": "🚀 Upgrade Gold (Premium)",
        "premium_msg": (
            f"<b>{E_MENU} UPGRADE PREMIUM</b>\n\n"
            f"💎 <b>Benefits:</b>\n"
            f"- Unlimited activations/day\n"
            f"- Faster processing priority\n"
            f"- Lifetime support\n\n"
            f"💰 <b>Price:</b> <code>{2000} VND</code> / Lifetime\n\n"
            f"👇 Click the button below to get payment info."
        ),
        "payment_instruction": (
            f"<b>{E_LOADING} PAYMENT INFORMATION</b>\n\n"
            f"🏦 <b>Bank:</b> {BANK_NAME}\n"
            f"👤 <b>Account Number:</b> <code>{BANK_ACCOUNT}</code>\n"
            f"💰 <b>Amount:</b> <code>{{}} VND</code>\n"
            f"📝 <b>Memo:</b> <code>{{}}</code>\n\n"
            f"⚠️ <b>NOTE:</b>\n"
            f"1. You must transfer the exact <b>Amount</b> and <b>Memo</b>.\n"
            f"2. System will upgrade automatically in 1-3 minutes.\n"
            f"3. Scan the QR code below for faster payment."
        ),
        "payment_success": f"{E_SUCCESS} <b>PAYMENT SUCCESSFUL!</b>\nYour account has been upgraded to <b>Premium</b>. You can now use unlimited activations!",
